...
5. 已完成：在「语音识别」页增加紧凑状态区，不新增侧栏入口，不显示技术版本日期。
6. 已完成：后台 updater 支持 HTTPS 同源下载、ETag、每日最多检查一次、手动强制检查和失败保留旧包；生产公钥、确定性生成/签名脚本、Pages 发布及 App 内存词典接入均已完成。
7. 已完成：增加原子替换、回滚、离线和性能测试；线上词包已从公开地址下载并由 App 自身校验器通过验签。
8. 已完成：真实语音回归覆盖热门名称、普通表达误伤和中英技术词；热门名称与误伤控制通过，长句中英切换存在一次偶发退化，缩短上下文后复测通过。
...
- 自动化测试确认无网络和更新失败不会打断输入，并继续使用上一有效词包。
- 添加用户词后，用户词优先于同名或近音热门词。
- 影视娱乐词只在相关场景提高命中，不污染技术文档。
- 没有有效热门词包时，原有识别能力保持不变。
