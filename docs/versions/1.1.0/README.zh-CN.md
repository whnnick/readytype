...
- 非邮件输出新增统一完整性规则，通用、聊天、笔记、文档和英文翻译不再无根据添加问候、致谢、确认、署名或收尾。
...
- 微信、TextEdit、双击 `Option`、`Esc` 取消和自动粘贴真实抽测已通过；浏览器、邮件和 AI 工具继续作为发布后的扩展抽测。

## 开发顺序

1. 常用词库产品化收口：已完成。
2. 确认式学习建议收口：已完成。
3. App 场景语气优化：已完成自动化与真实微信/TextEdit 回归。
4. 快捷键自定义体验复核：已完成自动化覆盖；真实双击 `Option` 与 `Esc` 取消通过。
5. 高精度语音包状态和更新提示：已完成自动化、在线检查和真实 App 回归。
6. 文档、测试和发布准备：进入 `1.1.0 (68)` 最终发布门禁。
