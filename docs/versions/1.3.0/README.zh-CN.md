...
已完成事件白名单、设置开关、核心输入漏斗、TelemetryDeck Provider、No-op 默认实现和官方服务端验收。Test Mode 已显示真实 `app_launched` 事件，事件详情字段审计未发现任何被禁止的内容字段；未注入配置的源码构建继续保持不发送。`1.3.0 (87)` 的全量测试、正式统计配置构建、ZIP、DMG、八页面 UI 和敏感信息检查已通过，剩余 GitHub Release 收口。
